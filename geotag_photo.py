...
class GeotagPhotos:
    ...
    def find_closest_gps(self, photo_datetime, max_time_diff_minutes=5):
        """
        查找最接近照片时间的GPS坐标

        Args:
            photo_datetime: 照片拍摄时间（UTC）
            max_time_diff_minutes: 最大时间差（分钟）
        ...
        # 找到最小时间差的记录
        closest_record = self.gps_data.loc[self.gps_data["time_diff"].idxmin()]
        ...
        if time_diff_minutes <= max_time_diff_minutes:
            ...
            print(
                f"最接近的GPS记录时间差为 {time_diff_minutes:.1f} 分钟，超过阈值 {max_time_diff_minutes} 分钟"
            )
            return None
    ...
    def process_photos(self, max_time_diff_minutes=5):
        """
        处理文件夹中的所有照片
        ...
        for filename in os.listdir(self.photo_folder):
            if filename.lower().endswith(supported_formats):
                ...
                gps_coords = self.find_closest_gps(
                    photo_datetime, max_time_diff_minutes
                )
                ...
                print(f"匹配的GPS坐标: {latitude:.6f}, {longitude:.6f}")
...
def main():
    ...
    parser.add_argument(
        "--max-time-diff", type=int, default=10, help="最大时间差（分钟），默认10分钟"
    )
    ...
    geotagger.process_photos(args.max_time_diff)
